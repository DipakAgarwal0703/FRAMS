...
from Admin_main import Admin_Section
import os
import time
...
class Admin_Login:
    ...
    def login(self):
        if (self.txtuser.get()=="" or self.txtpwd.get()==""):
            ...
            mycursor.execute("select * from admin_register where Email=%s and Password=%s",(
                self.txtuser.get(),
                self.txtpwd.get()
            ))
            ...
            if row==None:
                ...
                if open_min>0:
                    ...
                    self.app=Admin_Section(self.new_window)
                    # root.withdraw()
                    # root.destroy()
                else:
                    if not open_min:
...
#=======================Reset Passowrd Function=============================
    def reset_pass(self):
        if self.var_ssq.get()=="Select":
            ...
            conn = mysql.connector.connect(host="localhost",username="root",password="dipak0703@AG",database="face_recognition")
            mycursor = conn.cursor()
            query=("select * from admin_register where Email=%s and Security_Q=%s and Security_A=%s")
   ...
   #==============================Delete Function=========================================
        ...
        if self.txtuser.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(host="localhost",username="root",password="dipak0703@AG",database="face_recognition")
                    mycursor = conn.cursor() 
                    sql="delete from admin_register where Email=%s"
    ...
    def reg(self):
        self.new_window=Toplevel(self.root)
        self.app=Admin_Register(self.new_window)
